...
class FirestoreProducts {
  ...
  Future<void> saveCartItems(List<Product> cartItems) async {
    ...
    final List<Map<String, dynamic>> cartData = cartItems.map((product) {
      String imagesString = product.images.join(',');
      return {
        'name': product.name,
        'images': imagesString,
        'price': product.price,
    ...
    await cartCollection.doc().set(cartData.first);
  }

  Future<List<Product>> getCartProducts() async {
    final QuerySnapshot querySnapshot =
        await firestore.collection('cart').get();
    List<Product> cartProducts = querySnapshot.docs.map((doc) {
      final imagesData = doc.get('images');
      List<String> images;
      if (imagesData is String) {
        images = [imagesData];
    ...
    print('Productos del carrito obtenidos: $cartProducts');
    return cartProducts;
  }
}
